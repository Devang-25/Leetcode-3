...
def clean_up_href(href):
  ...
  sharp_pos = href.find('#')
  if sharp_pos != -1:
    href = href[:sharp_pos + 1] + href[sharp_pos + 1:].replace('#', '%23')
  return href
...
def get_urls_from_page(paths_to_follow, url):
  ...
  for href in paths_to_follow:
    ...
    if any(not re.search(valid_section_regex, part) for part in child_url_list):
      errors.append(
          "The page {0} has an href of {1}, which is not a valid URI".format(
              url, href))
      continue
    ...
    if not child_url_list[1]:
      child_url_list[2] = absolutize_path(child_url_list[2],
                                          parent_url_tuple[2])
...
def get_url_strings_from_doc(body_str):
  return (get_srcs_from_script_tags(body_str) +
          get_hrefs_from_link_tags(body_str) + get_hrefs_from_a_tags(body_str))
