...
def drawLine(x1, y1, x2, y2):
  # assume implemented by pramp

def drawHtree(x, y, length, depth):
  if not depth:
    return
  shrift = length/2
  center_line = drawLine(x-shrift, y, x+shrift, y)
  left_line = (x-shrift, y-shrift, x-shrift, y+shrift)
  right_line = (x+(shrift, y-shrift, x+shrift, y+shrift)
  depth -= 1
  if depth != 0:
    # top-left
    drawHtree(x-shrift, y+shrift, length/(sqrt(2)), depth)
    # bottom left
    drawHtree(x-shrift, y-shrift, length/(sqrt(2)), depth)
    # top right
    drawHtree(x+shrift, y+shrift, length/(sqrt(2)), depth)
