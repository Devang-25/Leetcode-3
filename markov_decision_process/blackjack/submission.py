    ...
    # 如果action是拿牌，上一轮没有peek, 还有牌
      ...
      for i in range(len(deckCardCounts)):
        if deckCardCounts[i] > 0:
          ...
          deckCardCounts_new = deckCardCounts[:]
    ...
    # action要看牌，没看过牌， 还有牌
      ...
      for i in range(len(deckCardCounts)):
        if deckCardCounts[i] > 0:
          ...
          deckCardCounts_new = deckCardCounts[:]
          deckCardCounts_new[i] -= 1
          future_state = (totalCardValueInHand, i, deckCardCounts_new)
          results.append((prob, self.R(state, action), future_state))
      self.peeked = True
...
class QLearningAlgorithm(util.RLAlgorithm):
  ...
  def incorporateFeedback(self, state, action, reward, newState):
    ...
    Q_est_opt_pred = self.getQ(state, action)  # return a score

    if newState is not None:
      # V_est_opt is a score, single int
      V_est_opt = max(
          self.getQ(newState, action) for action in self.getAction(newState))
      target = reward + self.discount * V_est_opt  # score, an single int

    for f, v in self.featureExtractor(state, action):
      self.weights[f] -= eta * (Q_est_opt_pred - target) * v
    # END_YOUR_CODE
